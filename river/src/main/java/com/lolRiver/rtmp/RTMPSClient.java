...
public class RTMPSClient {
    private static final Logger LOGGER = Logger.getLogger(RTMPSClient.class.getName());
    ...
    public void reconnect() {
        ...
        while (!isConnected()) {
            try {
                ...
                LOGGER.error("Error when reconnecting: ");
    ...
    private void getCertificate() throws IOException {
        try {
            ...
            if (!success) {
                ...
                out.close();
                LOGGER.debug("Installed cert for " + server);
        ...
        private void parsePackets(Thread thread) {
            try {
                ...
                while (true) {
                    ...
                    // Receive handler
                    if (id == null || id == 0) {
                        System.out.println("id was null, so no callback");
                    ...
                    // Callback handler
                        ...
                        System.out.println("callbacks had id, but got null when I removed it, so no callback");
                    } else {
                        results.put(id, result);
                        System.out.println("id was non null and non zero, but callbacks didnt contain, " +
                                           "so no callback");
                    }
                    pendingInvokes.remove(id);
                }
